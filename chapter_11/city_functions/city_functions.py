...
wrote. Write a function called test_city_country() to verify that calling 
your function with alues such as 'santiago' and 'chile' results in the correct 
string. Run the test, and make sure test_city_country() passes.

11-2. Population: pg 217
...
xxx', such as Santiago, Chile - population 5000000. Run the test again, 
and make sure test_city_country() fails this time.
    Modify the function so the poplation parameter is optional. Run the 
...
def city_country(city, country, population=''):
    cc = f"{city}, {country}".title()
    if population:
        cc += f" - population {population}"
    return cc